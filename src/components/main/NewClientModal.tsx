...
const NewClientModal: React.FC<NewClientModalProps> = ({ onClose, onSave }) => {
  ...
  return ReactDOM.createPortal(
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white w-[800px] max-h-[90vh] overflow-y-auto rounded shadow p-6">
        <h2 className="text-xl mb-4">거래처 신규등록</h2>
        <table className="w-full table-fixed border-collapse text-sm">
          <tbody>
            <tr>
              <td className="bg-gray-50 border px-2 py-2 font-semibold">사업자등록번호*</td>
            ...
            {/* 주민등록번호 */}
            <tr>
              <td className="bg-gray-50 border px-2 py-2 font-semibold">주민등록번호*</td>
              ...
              <td className="border px-2 py-2 font-semibold bg-gray-50">주민기재분*</td>
              <td className="border px-2 py-2">
            ...
            {/* 주소 */}
            <tr>
              <td className="bg-gray-50 border px-2 py-2 font-semibold">주소*</td>
              <td className="border px-2 py-2" colSpan={3}>
                <div className="flex gap-2 mb-2">
                  <input
                    ...
                    readOnly
                  />
                  <button className="px-3 py-1 border rounded hover:bg-black hover:text-white" onClick={callPostcode}>우편번호 조회</button>
                </div>
                <input
                  type="text"
            ...
            {/* 입금계좌 */}
            <tr>
              <td className="bg-gray-50 border px-2 py-2 font-semibold">입금 계좌</td>
              <td className="border px-2 py-2" colSpan={3}>
                <div className="flex gap-6 items-center mb-2">
                  <label className="w-24">은행</label>
                  <input type="text" className="w-[200px] border px-2 py-1 rounded" value={company.bank || ""} onChange={e => handleChange("bank", e.target.value)} />
                  <label className="w-24">예금주</label>
                  <input type="text" className="w-[200px] border px-2 py-1 rounded" value={company.account_holder || ""} onChange={e => handleChange("account_holder", e.target.value)} />
                </div>
                <div className="flex gap-2 items-center">
                  <label className="w-24">계좌번호</label>
                  <input type="text" className="flex-1 border px-2 py-1 rounded" value={company.account_number || ""} onChange={e => handleChange("account_number", e.target.value)} />
                </div>
              </td>
            </tr>
            {/* 계약일 */}
            <tr>
              <td className="bg-gray-50 border px-2 py-2 font-semibold">거래처 시작(종료)일</td>
              <td className="border px-2 py-2" colSpan={3}>
            ...
            {/* 비고 및 사용여부 */}
            <tr>
              <td className="bg-gray-50 border px-2 py-2 font-semibold">비고</td>
              <td className="border px-2 py-2" colSpan={3}>
                <textarea className="w-full border px-2 py-1 rounded" rows={2} value={company.note || ""} onChange={e => handleChange("note", e.target.value)} />
              </td>
            </tr>
            <tr>
              <td className="bg-gray-50 border px-2 py-2 font-semibold">사용여부</td>
              <td className="border px-2 py-2" colSpan={3}>
                <label className="mr-4">
        ...
        <div className="flex justify-end gap-2 mt-4">
          <button onClick={() => { onClose(); /* 상태 초기화도 같이 원하시면 여기서 초기화 로직 추가 */}} className="px-6 py-2 border rounded">취소</button>
          ...
          }} className={`px-6 py-2 ${canSave ? "bg-black text-white" : "bg-gray-300 text-gray-600"} rounded`}>
            저장
          </button>
        </div>
      </div>
